...
Original file is located at
    https://colab.research.google.com/drive/1ShiB7_inBmo2GSBzx14iNM4WSETQPKns
...
class RotNet(nn.Module):
	...
	# The model is modified to have 4 output features.
...
# Weight changes for two stages will be plotted: "SSL" and "Fine-tuning".
...
# Rotation is carried out as a combination of flipping and transposing.
...
class CovidCTDataset(Dataset):
	...
	# 4 possible rotations and their labels are packed into one sample.
...
def train(epoch):
	...
	# CrossEntropyLoss returns an average of all losses.
	# In this case, we need an average of those averages.
...
# Since RotNet is a classification task, it is easy to plot Grad-CAMs.
# For 3 images, 4 possible rotations and their Grad-CAMs are plotted.
model = model.cpu()
...
# The state of the model after the SSL stage is saved.
...
class FineTune(torch.nn.Module):
	...
	# RotNet pre-trained encoder is imported.
...
if train_epochs_start > 0:
    ...
    # The RotNet pre-trained model is imported from a checkpoint.
    model.load_state_dict(
        torch.load('ssl_backup/{}_{}.pt'.format(
            modelname,
            ssl_epochs
