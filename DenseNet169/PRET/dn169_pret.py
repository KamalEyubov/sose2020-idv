...
os.mkdir('model_backup') # will contain checkpoints for the fine-tuning stage 
os.mkdir('model_result') # will contain logs
...
device = 'cuda'
batchsize = 16
# In case if the execution is interrupted,
# train_epochs_start should be set to the last checkpoint,
# and some lines of the code should to be changed.
...
class FineTune(nn.Module):
    def __init__(self, out_dim=2):
        ...
        densenet = DenseNet169(pretrained=True) # a pre-trained DenseNet169 is imported
        self.features = densenet.features # we keep its encoder part
        self.relu = nn.ReLU(inplace=True) # ReLU is exposed (later useful for Grad-CAM)
        self.classifier = nn.Linear(
            densenet.classifier.in_features,
            out_features=out_dim,
            bias=True
        ) # the dimensions of the output are changed

    def forward(self, x):
	# the forward method is rewritten.
        out = self.features(x)
        out = self.relu(out)
...
if train_epochs_start > 0:
    # in case of an interrupted training, the last checkpoint is loaded
...
# The following lines and the function are for plotting the change of weights
# or the difference of parameters before and after a certain training stage. 
stages = ['fine-tuning']
stage_id = 0
...
def plot_change(model1, model2):
    ...
    # BN layers are plotted separately from the convolutional layers
    # owing to the prominent difference of their scales.
...
# The state of the model before fine-tuning is saved.
model_old = FineTune()
model_old.load_state_dict(model.state_dict())

# We use the mean and standard deviation used
# for COVID-CT in https://github.com/UCSD-AI4H/COVID-CT.
...
# detransformer converts a normalized tensor of an image back to its source image (useful for Grad-CAM).
...
def gradcam(model, input_tensor, detransform, label, axis):
	...
	frw_handle = model.relu.register_forward_hook(get_activations("features")) # activations of the ReLU layer are saved
	bck_handle = model.relu.register_backward_hook(get_grads("features")) # gradients received by the ReLU layer are saved

	model.eval()
	model.zero_grad()
	output_batch = model(input_batch)
	output_batch[0][label].backward() # gradients for a certain class are computed
...
class CovidCTDataset(Dataset):
    def __init__(self, root_dir, txt_COVID, txt_NonCOVID, transform=None):
        ...
        self.img_list = [] # file names along with their respective classes are saved in this list
...
bs = 10 # period at which the batch progress is displayed
votenum = 5 # period at which the validation performance is logged and plotted and a training checkpoint period
...
# The performance on the validation set is plotted.
...
# The loss on the training set (average for each epoch) is plotted.
...
# Performance on the test set is measured.
...
# Grad-CAM is plotted.
# 4 plots for positive cases and 4 for negative cases regardless of the model prediction.
model = model.cpu()
...
# The final state of the model is saved.
